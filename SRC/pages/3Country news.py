...
if choosen_company is not None:
    ...
    st.markdown(
        "<h3 style='text-align: center; color: #04d995;text-decoration: underline'>Country News & Sentiment Analysis</h3>",
        unsafe_allow_html=True,
    )
    country_company_news = get_news(
        json_data["news"]["country_news"],
    )
    
    st.data_editor(
        country_company_news,
        column_config={
            "url": st.column_config.LinkColumn("url"),
            **{col: st.column_config.Column(width="small") for col in country_company_news.columns if col != "url"}
        },
        hide_index=True,
        height=800,
        num_rows="fixed",
